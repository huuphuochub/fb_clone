...
export class HomeComponent implements OnInit {
  ...
  hoidanggi(){
    this.ishoidanggi = true
  }
  closehoidanggi(){
    this.ishoidanggi = false;
  }
  openformpost(){
    this.formpost = true;
    this.ishoidanggi = false;
...
handleaddpost(){
  ...
  formdataa.append('id_user',this.id_user);
  formdataa.append('content',this.formaddpost.get('content')?.value);
  formdataa.append('status',this.selectedoption);
  formdataa.append('file',img);
  this.Postservice.addpost(formdataa)
  .pipe(
    catchError((error:any) =>{
      // console.error('eroor');
      const message = 'lỗi khi tạo bài viết';
  ...
  .subscribe(data =>{
    ...
    if(data === true){
      ...
      this.selectedoption ='';
      this.formpost = false;
      this.showNotification('bạn đã đăng bài viết thành công')
    }
  })
